...
Dashboard em Streamlit para análise descritiva de resultados da roleta
europeia (0–36).

Giros de roleta são independentes. O projeto não garante previsão, vantagem
ou lucro.

Melhorias da versão 2
HTML renderizado com st.html(), evitando tags exibidas como código.
Interface principal reduzida e organizada em funções.
Motor separado da interface.
Comparação dos setores ajustada pelo tamanho teórico de cada setor.
Z-score com tamanho efetivo da amostra ponderada.
Epicentro escolhido pela concentração da vizinhança física, não apenas pela
repetição do número central.
Backtest walk-forward.
Intervalo de Wilson de 95% para a taxa observada.
Exportação do histórico em CSV.
...
A leitura compara a participação recente de cada setor com sua participação
teórica na roda. O score exibido é uma métrica interna de evidência histórica,
não uma probabilidade de acerto no próximo giro.
